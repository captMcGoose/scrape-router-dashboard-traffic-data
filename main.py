...
# Abrimos el csv
with open('datosRouter.csv', mode='w', newline='') as file:
    ...
    for i in range(24):
        ...
        extractedNumber = int(rows[6].find_all('td')[1].text) + int(rows[7].find_all('td')[1].text) + int(rows[8].find_all('td')[1].text) + int(rows[9].find_all('td')[1].text) + int(rows[10].find_all('td')[1].text) + int(rows[12].find_all('td')[1].text) 
